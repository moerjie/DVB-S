...
architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_PolarityShift is
  signal \dataTemp[1]_i_1_n_0\ : STD_LOGIC;
begin
\dataTemp[1]_i_1\: unisim.vcomponents.LUT1
    generic map(
      INIT => X"1"
    )
        port map (
      I0 => rst_n,
      O => \dataTemp[1]_i_1_n_0\
    );
\dataTemp_reg[0]\: unisim.vcomponents.FDCE
     port map (
      C => clk,
      CE => '1',
      CLR => \dataTemp[1]_i_1_n_0\,
      D => '1',
      Q => doubleOut(0)
    );
...
inst: entity work.decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_PolarityShift
     port map (
      clk => clk,
      doubleOut(1 downto 0) => doubleOut(1 downto 0),
      rst_n => rst_n,
      singleIn => singleIn
    );
end STRUCTURE;
